...
def genR(request):
    ...
    cursor = conn.cursor()
    request.session['rollno'] = '15CE5003'
...
def uploadLetter(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            query="insert into offer_letter (roll_no_fk,company,date,doc_name) values('"+request.session.get('rollno',"")+"','"+companyName+"','"+dateT+"','"+fname+"')"
            print()
            cursor.execute(query)
            form = DocumentForm()
...
def details(request):
    ...
    if request.method == 'POST':
        ...
        conn=connections['default']
        query = "update student set name='"+name+"',year='"+year+"',branch='"+branch+"',address='"+address+"',contact='"+contact+"',dob='"+dob+"' where rollno = '"+request.session['rollno']+"' "
        cursor = conn.cursor()
        cursor.execute(query)
        context={
            'result':result,
...
def other_details(request):
    ...
    if request.session['rollno'] is None:
        ...
        if request.method == 'POST':
            ...
            query="update student_academic set ssc="+ssc+",hsc="+hsc+",s1="+s1+",s2="+s2+",s3="+s3+",s4="+s4+",s5="+s5+",s6="+s6+",s7="+s7+",s8="+s8+",live_kt="+live_kt+",dead_kt="+dead_kt+",objective='"+objective+"',extra='"+extra+"',achievement='"+achievement+"' where roll_no_fk = '"+request.session['rollno']+"' "
            print(query)
            cursor.execute(query)
            context={
                'rollno': request.session['rollno'],
...
def display_events(request):
    ...
    query1="select ename,description,date,from_hr,to_hr,amount,author,eid from events where target_aud = '"+args[1]+"' and branch = '"+args[0]+"'"
    print(query1)
    cursor.execute(query1)
    events=cursor.fetchall()
    print(events)
    print(args)
    query2="select * from formregister where eid="+events[7]+" and name='"+args[2]+"'"
    cursor.execute(query1)
